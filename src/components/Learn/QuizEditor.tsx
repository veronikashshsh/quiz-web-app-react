...
function QuizEditor() {
  ...
  const { localQuiz, addFlashcard, deleteFlashcard } = useQuizEditor(foundQuiz as Quiz, isGuest);
    ...
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-1">
          {localQuiz.flashcards.length} {t('quiz.cards', { count: localQuiz.flashcards.length })}
        </p>
      </div>

      {/* Form */}
      <form onSubmit={handleSubmit} className="bg-gray-100 border border-gray-200 rounded-xl p-6 mb-8">
      ...
      {localQuiz.flashcards.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3">
          {localQuiz.flashcards.map((fc: Flashcard, i: number) => (
            <li key={i} className="flex items-center gap-4 px-5 py-4 bg-white border border-gray-200 rounded-xl hover:border-indigo-200 transition">
              <div className="flex flex-col gap-2 flex-1">
                <div className="flex items-start gap-2.5">
                  <span className="shrink-0 w-5 h-5 rounded bg-indigo-100 text-indigo-700 text-[11px] font-bold flex items-center justify-center mt-0.5">Q</span>
                  <span className="text-sm text-gray-700">{fc.question}</span>
                </div>
                <div className="flex items-start gap-2.5">
                  <span className="shrink-0 w-5 h-5 rounded bg-emerald-100 text-emerald-700 text-[11px] font-bold flex items-center justify-center mt-0.5">A</span>
