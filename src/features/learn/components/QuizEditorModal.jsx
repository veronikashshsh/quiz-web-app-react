      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-7xl p-8 relative">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-400">
            {localQuiz.flashcards.length} {localQuiz.flashcards.length === 1 ? 'card' : 'cards'}
          </p>
        </div>
        ...
        {localQuiz.flashcards.length === 0 ? (
          <div className="text-center text-gray-400 text-sm py-6 border border-dashed rounded-lg">No cards yet</div>
        ) : (
          <div className="flex flex-col gap-3 max-h-64 overflow-y-auto pr-1">
            {localQuiz.flashcards.map((fc, i) => (
              <div key={i} className="flex justify-between items-start gap-4 p-4 border border-gray-200 rounded-xl bg-white hover:shadow-sm transition">
                <div className="flex flex-col gap-1">
                  <p className="text-sm text-gray-800"><span className="font-semibold text-indigo-600">Q:</span> {fc.question}</p>
                  <p className="text-sm text-gray-500"><span className="font-semibold text-emerald-600">A:</span> {fc.answer}</p>
                </div>
                <button onClick={() => deleteFlashcard(i)} className="text-red-400 hover:text-red-600 text-lg">×</button>
              </div>
            ))}
          </div>
        )}
